...
$$
\mathbf{A} \mathbf{b} = \begin{pmatrix} - & \mathbf{a}_1' & - \\
& \vdots & \\ - & \mathbf{a}_m' & - 
\end{pmatrix} \begin{pmatrix} | \\ \mathbf{b} \\ | \end{pmatrix} = \begin{pmatrix} \mathbf{a}_1' \mathbf{b} \\ \vdots \\ \mathbf{a}_m' \mathbf{b} \end{pmatrix} \in \mathbb{R}^m.
$$
...
\begin{eqnarray*}
    ...
    \end{cases}.
\end{eqnarray*}
Note this definition differs from the BV book (p112), which defines $a_{ij}=1$ when there is an edge from node $j$ to node $i$.
...
- Example: $n=4$, $m=3$,
$$
\mathbf{a} = \begin{pmatrix} a_1 \\ a_2 \\ a_3 \\ a_4 \end{pmatrix}, \quad \mathbf{b} = \begin{pmatrix} b_1 \\ b_2 \\ b_3 \end{pmatrix}.
$$
...
- Propoerties of convolution
    ...
    These properties follow either from the polynomial interpretation or the probabilistic interpretation.
...
- Thus overall Gram-Schmidt algorithm produces
$$
