...
- Why are all eigenvalues of a square matrix less than or equal to $\sigma_1$? 
    ...
    Answer: Recall that orthogonal matrices preserve vector length. Using the full SVD $\mathbf{A} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}'$, 
$$
...
Using the SVD, the [Moore-Penrose inverse](https://ucla-biostat-216.github.io/2024fall/slides/07-matinv/07-matinv.html#generalized-inverse-and-moore-penrose-inverse) is given by
$$
...
- Let's first look at an image compression example from <https://www.mathworks.com/company/technical-articles/professor-svd.html?s_tid=srchtitle>.
...
Thus the optimal $\mathbf{x}$ must be an eigenvector of $\mathbf{S} = \mathbf{A}' \mathbf{A}$ with corresponding eigenvalue $f(\mathbf{x})$. Which one shall we choose? Apparently the **top right singular vector** $\mathbf{x}^\star = \mathbf{v}_1$ gives us the maximal value which is equal to $\lambda_1 = \sigma_1^2$.
    ...
    In HW7 Q3, you show this result using an alternative proof (elementary linear algebra arguments).
...
$$
    ...
    Proof: HW7 Q6. Hint: use Rayleigh quotient.
